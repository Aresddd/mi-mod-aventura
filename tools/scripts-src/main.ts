/**
 * El Sifón - Main Script Entry Point (TypeScript Source)
 *
...
export type QuestPhase = typeof QUEST_PHASES[keyof typeof QUEST_PHASES];
...
interface QuestUpdateEvent {
  player: Player;
  phase: QuestPhase;
  previousPhase?: QuestPhase;
}
 ...
 */
export function getPlayerQuestPhase(player: Player): QuestPhase {
  return (player.getDynamicProperty(PLAYER_PROPS.QUEST_PHASE) as QuestPhase) || QUEST_PHASES.INICIO;
}

/**
 * Establece la fase de la misión del jugador
 */
export function setPlayerQuestPhase(player: Player, phase: QuestPhase): void {
  const previousPhase = getPlayerQuestPhase(player);
  player.setDynamicProperty(PLAYER_PROPS.QUEST_PHASE, phase);
  sendQuestUpdate(player, phase, previousPhase);
}

/**
 * Verifica si el jugador tiene un ítem de progresión
 */
export function hasProgressionItem(player: Player, itemId: string): boolean {
 ...
 */
export function giveProgressionItem(player: Player, itemId: string, count = 1): boolean {
  const inventory = player.getComponent("minecraft:inventory");
  if (!inventory) return false;
 ...
 */
function sendQuestUpdate(player: Player, phase: QuestPhase, previousPhase?: QuestPhase): void {
  // Enviar evento al cliente para actualizar HUD/UI
  player.runCommand(`scriptevent ${NAMESPACE}:quest_update ${phase}`);

  // Log para debugging
  console.log(`[${NAMESPACE}] Player ${player.name} quest phase: ${previousPhase || "none"} -> ${phase}`);
}

/**
 * Maneja la muerte de un jefe principal
 */
export function handleBossDefeat(player: Player, bossType: string): void {
 ...
 */
export function initializePlayerProgress(player: Player): void {
 ...
 */
function getQuestPhaseValue(phase: QuestPhase): number {
  const values: Record<QuestPhase, number> = {
...
world.afterEvents.entityDie.subscribe((event) => {
  ...
  if (killer && killer.typeId === "minecraft:player") {
    ...
    if (typeFamily?.hasTag("sifon_corrupted")) {
 ...
 */
function handleSifonCommand(player: Player, args: string[]): void {
  ...
  switch (subcommand) {
    ...
    case "spawn":
      if (player.hasTag("admin") || player.isOp()) {
        ...
        if (entityType && Object.values(ENTITY_IDS).includes(`${NAMESPACE}:${entityType}` as any)) {
 ...
 */
function showProgressUI(player: Player): void {
  const phase = getPlayerQuestPhase(player);
  const phaseNames: Record<QuestPhase, string> = {
 ...
 */
function resetPlayerProgress(player: Player): void {
 ...
 */
function spawnEntityNearPlayer(player: Player, entityType: string): void {
