/**
 * El Sifón - Definición de Ítems Personalizados
 *
 * Este archivo define todos los ítems personalizados del addon usando
 * la Script API para registro de ítems personalizados.
 */

import { world, ItemStack, ItemComponentTypes, ItemType } from "@minecraft/server";
import { NAMESPACE, PROGRESSION_ITEMS } from "./main.js";
 ...
 */
export function registerCustomItems() {
 ...
 */
function registerItem(itemId, config) {
 ...
 */
function registerAntiCorruptionGear() {
  const gearItems = {
 ...
 */
export function isProgressionItem(itemId) {
  return Object.values(PROGRESSION_ITEMS).includes(itemId);
}
